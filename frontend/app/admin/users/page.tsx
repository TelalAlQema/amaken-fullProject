...
const TABS = [
  { label: "All", href: "/admin/users", icon: Users },
  { label: "Users", href: "/admin/users?filter=user", icon: Users },
  { label: "Agents", href: "/admin/users/agents", icon: UserCheck },
  { label: "Builders", href: "/admin/users/builders", icon: Building2 },
];
...
export default function AdminUsersPage() {
  ...
  const { data: response, isLoading } = useQuery<ApiResponse<UsersResponse>>({
    ...
    queryFn: () =>
      api
        .get("/admin/users", { params: { page, limit: 50 } })
        .then((r) => r.data),
  });
  ...
  const filtered = items.filter((u) => {
    if (activeFilter !== "all" && u.utype?.toLowerCase() !== activeFilter)
      return false;
    if (search) {
        ...
        {TABS.map((tab) => (
          <Link
            ...
            className={`inline-flex items-center gap-1.5 rounded-lg px-4 py-2 text-sm font-medium transition-colors ${
              activeFilter === tab.label.toLowerCase()
                ? "bg-[#17c788] text-white"
                : "bg-white text-gray-600 border border-gray-200 hover:bg-gray-50"
            }`}
            onClick={(e) => {
              if (tab.href.includes("?")) {
                e.preventDefault();
                setActiveFilter(tab.label.toLowerCase());
                setPage(1);
              }
            }}
          >
