...
export default function BlockedAccountsPage() {
  ...
  const { data: response, isLoading } = useQuery<ApiResponse<DelAccount[]>>({
    queryKey: ["admin-blocked-accounts", page],
    queryFn: () =>
      api
        .get("/admin/accounts/blocked", { params: { page, limit: 50 } })
        .then((r) => r.data),
  });
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["admin-blocked-accounts"] });
      setDeleteId(null);
    },
  });
  ...
  return (
    <div className="bg-gray-50 p-6 min-h-screen">
      <ConfirmDialog
        ...
        message="Are you sure you want to remove this block record? This action cannot be undone."
        onConfirm={() => {
      ...
      <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500">
            {total} blocked account{total !== 1 ? "s" : ""}
          </p>
        </div>
      </div>
